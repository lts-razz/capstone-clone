  ...
  try {
    ...
    if (paid) {
      const { data: booking, error: bookingError } = await db.from("bookings")
        .select("id,total_price,minimum_payment_amount,status").eq("id", priorTx.booking_id).single();
      ...
      if (booking) {
        ...
        const { data: previousPayment, error: paymentFetchError } = await db.from("booking_payments")
          .select("amount_paid,payment_status,minimum_payment_amount,total_booking_amount,payment_method").eq("booking_id", booking.id).maybeSingle();
        ...
        if (hasRequiredSecuringPayment(booking, paymentState) && (booking.status === "pending" || booking.status === "rescheduled")) {
